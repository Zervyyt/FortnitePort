...
public class UnrealModel : IRenderable
{
    ...
    public UnrealModel(CSkeletalMesh convertedMesh)
    {
        ...
        for (int i = 0; i < lod.Indices.Value.Length; i++)
        {
            Indices.Add((uint) lod.Indices.Value[i]);
        }
        ...
        // TODO actual sections for multi-material meshes
        var test = lod.Sections.Value.First();
        var testmat = test.Material.Load<UMaterialInterface>();
        var parameters = new CMaterialParams2();
        testmat.GetParams(parameters, EMaterialFormat.AllLayers);
